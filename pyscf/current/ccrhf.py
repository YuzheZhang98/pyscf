...
'''
Current-Constrained Restricted Hartree-Fock (CC-RHF)
'''
...
def get_fock_add_ccrhf(ipovlp, c, length, direct):
...
class CCRHF(RHF):
    ...
    def get_fock_add_ccrhf(self):
        return get_fock_add_ccrhf(self.ipovlp, self.c,
                                  self.length, self.current_dir)

    def get_fock(self, h1e=None, s1e=None, vhf=None, dm=None, cycle=-1,
                 diis=None, diis_start_cycle=None,
                 level_shift_factor=None, damp_factor=None):
        f0 = RHF.get_fock(self, h1e=h1e, s1e=s1e, vhf=vhf,
                          dm=dm, cycle=cycle, diis=diis,
                          diis_start_cycle=diis_start_cycle,
                          level_shift_factor=level_shift_factor,
                          damp_factor=damp_factor)
        return f0 + self.get_fock_add_ccrhf()
