...
class _DFNEO:
    ...
    def get_vint(self, mol=None, dm=None, **kwargs):
        ...
        for (t1, t2), interaction in self.interactions.items():
            ...
            if t1.startswith('n') and t2.startswith('n'):
        ...
        for t, mf in mf_n.items():
            cderi[t], low = _build_cderi(mf_e, mf, mol.cart,
                                         self.max_memory, mol.verbose)
            if mf_e.with_df._low is None:
                mf_e.with_df._low = low

        if dm['e'] is not None:
            t0 = (logger.process_clock(), logger.perf_counter())
            if dm['e'].ndim > 2:
                dms1 = numpy.asarray(dm['e'])
                dms1 = dms1[0] + dms1[1]
            else:
                dms1 = numpy.asarray(dm['e'])
            ...
            tmp = dmtril.dot(mf_e._cderi.T)
            for t, eri in cderi.items():
                vj = tmp.dot(eri)
                nao = dm[t].shape[-1]
                if len(dm_shape1) == 2:
                    vint[t] += lib.unpack_tril(vj, 1).reshape((nao, nao)) * mf_n[t].charge
                else:
                    vint[t] += lib.unpack_tril(vj, 1).reshape((-1, nao, nao)) * mf_n[t].charge
            logger.timer(self, 'df vj between e and all n', *t0)

        t0 = (logger.process_clock(), logger.perf_counter())
        tmp = 0
        for t in mf_n.keys():
            if dm[t] is not None:
                dms1 = numpy.asarray(dm[t])
                dm_shape1 = dms1.shape
                nao1 = dm_shape1[-1]
                dms1 = dms1.reshape(-1,nao1,nao1)
                idx = numpy.arange(nao1)
                dmtril = lib.pack_tril(dms1 + dms1.conj().transpose(0,2,1))
                dmtril[:,idx*(idx+1)//2+idx] *= .5
                tmp += dmtril.dot(cderi[t].T) * mf_n[t].charge

        vj = tmp.dot(mf_e._cderi)
        nao = dm['e'].shape[-1]
        vint['e'] += lib.unpack_tril(vj, 1).reshape((nao, nao))
        logger.timer(self, 'df vj between all n and e', *t0)
